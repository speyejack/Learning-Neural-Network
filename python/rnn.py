...
class Layer:
    def __init__(self, input_size, output_size):
        ...
        self.weights = {"input": np.random.randn(output_size, input_size), "hidden": np.random.randn(output_size, output_size)}
        self.reset()
...
class HiddenLayer(Layer):
    ...
    def _forward_prop(self, input_v):
        a = self.weights["input"].dot(input_v) + self.weights["hidden"].dot(self.data["outputs"][-1])
        b = self.sigmoid(a)
        return b
